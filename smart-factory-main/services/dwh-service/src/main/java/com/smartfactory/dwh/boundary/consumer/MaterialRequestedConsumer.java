...
public class MaterialRequestedConsumer {
    private static final Logger LOG = Logger.getLogger(MaterialRequestedConsumer.class);

    private final DwhEventProcessingService dwhEventProcessingService;

    public MaterialRequestedConsumer(final DwhEventProcessingService dwhEventProcessingService) {
        this.dwhEventProcessingService = dwhEventProcessingService;
    }

    @Incoming(MATERIAL_REQUESTED)
    public void consume(final MaterialRequestedEvent event) {
        if (event == null) {
        ...
        LOG.infof("Received %s eventId = %s", MATERIAL_REQUESTED, event.event().eventId());
